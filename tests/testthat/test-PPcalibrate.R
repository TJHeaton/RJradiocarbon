test_that("PPcalibrate gives expected outcomes", {
  ...
  hist(PP_fit_output$n_internal_changes)
  min_cal_age <- min(PP_fit_output$rate_s[[1]])
  max_cal_age <- max(PP_fit_output$rate_s[[1]])
  n_out <- length(PP_fit_output$n_internal_changes)
