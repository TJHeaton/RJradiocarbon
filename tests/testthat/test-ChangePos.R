...
### Second test
test_that("ChangePos gives sameas legacy code", {

  set.seed(14)
  rate_s <- initial_rate_s <- c(0, 2, 5, 7, 10)
  rate_h <- initial_rate_h <- c(5, 2, 4, 6)
  n_changepoints <- length(rate_s)
  integrated_rate <- initial_integrate_rate <- .FindIntegral(
    initial_rate_s,
    initial_rate_h)
  ...
  for(i in 1:1000) {
    ...
    rate_s <- revised_rate_s <- return_val$s
  ...
  integrated_rate <- initial_integrate_rate
  for(i in 1:1000) {
